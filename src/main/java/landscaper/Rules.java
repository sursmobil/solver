...
public class Rules implements RuleSet<Board2d<Tile>>{
    ...
    private boolean checkSequences(List<Area<Tile>> areas) {
        BooleanSupplier singleCheckF = () -> {
            for(Area<Tile> s : areas) {
    ...
    private boolean noDuplicateSequence(List<Area<Tile>> areas) {
    ...
    private boolean validNumberOfTiles(Area<Tile> s) {
        int max = s.size()/2;
    ...
    private boolean noMoreThenTwoConsecutive(Area<Tile> s) {
    ...
    public Collection<Board2d<Tile>> singleProblemAlternatives(Board2d<Tile> state) {
        Area<Tile> row = state.rows().stream()
    ...
    private Collection<Board2d<Tile>> createAlternatives(Board2d<Tile> state, Area<Tile> row, List<Tile[]> perms) {
